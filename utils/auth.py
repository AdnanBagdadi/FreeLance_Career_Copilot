...
def _shared_css():
    st.markdown(
        ...
        .feat-card {
            ...
            background:white; height:170px;
        }
        .feat-card .emoji {font-size:26px;}
        .feat-card h4 {margin:10px 0 6px 0; font-size:16px; color:#0f172a;}
        .feat-card p {margin:0; font-size:12.8px; color:#6b7280; line-height:1.45;}
...
def _render_landing():
    ...
    st.markdown(
        ...
        tailored proposal — all in one pass.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    for row_start in range(0, len(FEATURES), 3):
        cols = st.columns(3)
        for col, (emoji, title, desc) in zip(cols, FEATURES[row_start:row_start + 3]):
            col.markdown(
                f"""
                <div class="feat-card">
