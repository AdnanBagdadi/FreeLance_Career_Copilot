...
def _render_login_form():
    ...
    # NFR: Security — brute-force lockout check with live ticking countdown
    now = time.time()
    lockout_until = st.session_state.get("lockout_until", 0)
    locked = now < lockout_until

    lockout_placeholder = st.empty()

    if locked:
        while time.time() < lockout_until:
            remaining = int(lockout_until - time.time())
            if remaining <= 0:
                break
            lockout_placeholder.warning(f"Too many failed attempts. Try again in {remaining}s.")
            time.sleep(1)
        
        lockout_placeholder.empty()
        st.session_state.login_attempts = 0
        st.session_state.lockout_until = 0
        locked = False

    with st.form("login_form", clear_on_submit=False):
        username = st.text_input("Username", placeholder="example@123", disabled=locked)
        password = st.text_input("Password", type="password", placeholder="••••••••", disabled=locked)
        submitted = st.form_submit_button("Log in", use_container_width=True, disabled=locked)
