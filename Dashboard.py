...
nav_left, nav_btn1, nav_btn2 = st.columns([4, 1, 1])
with nav_left:
    st.markdown("### 💼 Freelance Career Copilot")
with nav_btn1:
    if st.button("View Profile", key="navbar_view_profile", use_container_width=True):
        st.switch_page("pages/7_View_Profile.py")
with nav_btn2:
    if st.button("Missing Skills", key="navbar_missing_skills", use_container_width=True):
...
with c1:
    ...
    list_label = "Close list" if st.session_state.get("show_jobs_list", False) else "View list"
    if st.button(list_label, key="toggle_jobs_list"):
